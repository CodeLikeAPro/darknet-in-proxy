...
function proxify_url($url, $base_url = ''){
	...
	if($base_url){
		...
		//log::notice('$url after rel2abs: ' . $url);

		$base_host = parse_url($url, PHP_URL_HOST);
		//log::notice('$url PHP_URL_HOST: ' . $base_host);
...
function rel2abs($rel, $base)
{
	...
	$base_host = parse_url($url, PHP_URL_HOST);
	if($base_host != NULL) $host = $bash_host;
	/* dirty absolute  URL */
	$abs = "$host$path/$rel";
